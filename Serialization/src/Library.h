...
namespace Serialization
{
	...
	template<typename T>
	inline size_t GetElementSize(const T& obj)
	{
		return sizeof(T);
	};

	template<>
	inline size_t GetElementSize(const std::string& obj)
	{
		return obj.size();
	};

	template<typename T>
	inline size_t GetVectorSize(const std::vector<T>& obj)
	{
		return obj.size();
	};
	...
	{
		fs.read(reinterpret_cast<char*>(&o), GetElementSize(o));
	};
	...
	{
		fs.write(reinterpret_cast<const char*>(&o), GetElementSize(o));
	};

	template<>
	inline void WriteBinary(std::fstream& fs, const std::string& o)
	{
		size_t size = GetElementSize(o);
